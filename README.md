...
- [x] **Web UI** — Streamlit interface with copy-to-clipboard and guardrail styling
...
| **Streamlit UI** | `app.py` | Web interface; uses `streamlit_backend.py` for structured responses |
...
│   ├── streamlit_backend.py  # Adapter for Streamlit (path + explanation + code)
...
```
Then open http://localhost:8501. The UI uses the same pipeline (guardrails → retriever → generator) and shows explanation + code with a copy button.
...
| Limitation | Impact |
|------------|--------|
| **No API endpoint** | Web UI and CLI only; no REST API |
...
| Improvement | Benefit |
|-------------|---------|
| **REST API / Deploy** | Expose API or deploy Streamlit UI (e.g. Streamlit Cloud) |
